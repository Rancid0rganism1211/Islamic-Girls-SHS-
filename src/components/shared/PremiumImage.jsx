...
export default function PremiumImage({ src, alt, containerClassName = "", imgClassName = "", objectPosition = "object-center" }) {
  return (
    <div className={`rounded-2xl shadow-xl border border-emerald-950/10 overflow-hidden ${containerClassName}`}>
      <img
        src={src}
        alt={alt}
        className={`w-full h-full object-cover ${objectPosition} scale-[1.03] filter contrast-[1.02] saturate-[1.03] brightness-[0.98] ${imgClassName}`}
      />
    </div>
  );
}